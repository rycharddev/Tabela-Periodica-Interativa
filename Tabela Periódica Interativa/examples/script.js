...
const tabelaPeriodica = document.querySelector('.tabela-periodica');
const infoElemento = document.getElementById('info-elemento');

// Função para criar os elementos da tabela periódica
function criarElementos() {
    elementos.forEach(elemento => {
        const divElemento = document.createElement('div');
        divElemento.classList.add('elemento');
        divElemento.textContent = elemento.simbolo;
        divElemento.title = elemento.nome + ": " + elemento.exemplo;
        divElemento.addEventListener('click', () => exibirInfoElemento(elemento));
        tabelaPeriodica.appendChild(divElemento);
    });
}

// Função para exibir informações do elemento selecionado
function exibirInfoElemento(elemento) {
    infoElemento.innerHTML = `
        <h2>${elemento.nome} (${elemento.simbolo})</h2>
        <p>Exemplo do dia a dia: ${elemento.exemplo}</p>
    `;
}

// Chamando a função para criar os elementos da tabela periódica
criarElementos();
